...
export const updateSearchCount = async (searchTerm, movie) => {
    // Use Appwrite to check if the searchTerm already exists
    try {
        const result = await database.listDocuments(DATABASE_ID, COLLECTION_ID, [Query.equal('searchTerm', searchTerm)])

    //If it does we should updat ethe search count
        if(result.documents.length > 0) {
            const doc = result.documents[0];

            await database.updateDocument(DATABASE_ID, COLLECTION_ID, doc.$id, {count: doc.count + 1,})
    //if it doesnt we should create a new doc with the searchTerm and count as 1
        } else {
